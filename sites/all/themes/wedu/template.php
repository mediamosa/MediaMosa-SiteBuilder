 ...
 */
function wedu_process_html(&$variables) {
 ...
 */
function wedu_preprocess_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
...
function wedu_preprocess_user_profile(&$variables) {

  // Checks if user profile page exist
  if (arg(0) === 'user' && arg(1) > 0) {
    ...
    // For mediamosa only to see video's on user profile

    // Checks if user is logged in and shows assets
    if ($account !== FALSE) {
      $view = views_get_view('mediamosa_assets');
      if (!$view || !$view->access('block')) {
        return [];
      }
      $cql = strtr('owner_id == "^:owner_id^"', [':owner_id' => mediamosa_sdk::escape_cql($account->mail)]);
      $variables['media'] = $view->preview('block', ['cql' => $cql]);
    }
    else {
      return '-';
    }
 ...
 */
function wedu_breadcrumb($variables) {
 ...
 */
function wedu_mediamosa_ck_views_theme_asset_mediafiles($variables) {
 ...
 */
function wedu_mediamosa_ck_views_theme_owner($variables) {
  ...
  // Bug - return is missing
  return theme_mediamosa_ck_views_theme_owner($variables);
}
 ...
 */
function wedu_select($variables) {
  ...
  return '<select' . drupal_attributes($element['#attributes']) . '>'
    . wedu_form_select_options($element) . '</select>';
}
 ...
 */
function wedu_form_select_options($element, $choices = NULL) {
  if (!isset($choices)) {
 ...
 */
function wedu_mediamosa_ck_views_theme_asset_metadata($variables) {
  $rows = array();
